...
async function handleSaveContract() {
	...
	try {
		...
		if (isEditingContract.value && editingContractGuid.value) {
			...
			const { error } = await customerApi.createContract(customer.value.guid, contractForm.value);
			if (error) {
		...
		<div class="maintenance-view__header">
			<div class="maintenance-view__title-area">
				<button class="btn btn--text" style="color: var(--colors-brand-primary); padding: 0" @click="router.back()">
					<i class="mdi mdi-arrow-left"></i> Back to Customer Directory
				</button>
				<h1 class="mt-xs">Customer Profile</h1>
			</div>
			<div class="header-actions">
				<button class="btn btn--secondary mr-sm" @click="openAddContractModal">
					<i class="mdi mdi-file-plus-outline"></i> Add New Contract
				</button>
				...
				<div class="panel-card user-meta-card">
					...
					<div class="user-meta-card__badges mt-sm">
						<Badge :type="customer.requestEinvoice ? 'info' : 'default'">
							{{ customer.requestEinvoice ? "LHDN MyInvois Enabled" : "Standard Invoicing" }}
						</Badge>
					...
					<div class="quick-nav-box w-full mb-md">
						...
						<div class="contract-card mt-xs">
							...
							<span class="contract-card__code">
								{{ contracts[0]?.contractNo || customer.contractNo || customer.metadata?.contractNo || "No Contract Linked" }}
							</span>
						</div>
					</div>

					<!-- MSIC Industry Box -->
					<div v-if="customer.profile && customer.profile.msicCode" class="quick-nav-box w-full">
						<span class="quick-nav-box__label">MSIC Core Business</span>
						<div class="msic-display-box mt-xs">
							<span class="msic-display-box__code">{{ customer.profile.msicCode }}</span>
							<p class="msic-display-box__desc">{{ customer.profile.msicDesc || "General Business Activity" }}</p>
						</div>
					</div>
				</div>
			</div>
				...
				<div class="detail-stack">
					...
					<div class="panel-card panel-card--readonly">
							...
							>
								<div class="contract-item-card__main">
									...
									<div class="contract-item-card__dates mt-xs">
										<span><i class="mdi mdi-calendar-start"></i> Start: <strong>{{ formatDate(c.startDate) }}</strong></span>
										<span class="mx-xs">•</span>
										<span><i class="mdi mdi-calendar-end"></i> Expiry: <strong>{{ formatDate(c.endDate) }}</strong></span>
									</div>
						...
						<div v-else class="empty-contracts">
							...
							<div class="empty-contracts__action">
								<button class="btn btn--sm btn--primary" @click="openAddContractModal">
									<i class="mdi mdi-plus"></i> Create First Contract
								</button>
							</div>
						</div>
					</div>
					...
					<div class="panel-card panel-card--readonly mt-lg">
						<div class="panel-card__header">
							<h2>
								<i class="mdi mdi-account-box-outline u-text-primary"></i> Debtor Contact & Tax Identity
							</h2>
						</div>
						<div class="readonly-grid" v-if="customer.profile">
							<div class="readonly-item">
								<span class="readonly-item__label">Login Email</span>
								<span class="readonly-item__value">{{ customer.profile.email || "—" }}</span>
							</div>
							<div class="readonly-item">
								<span class="readonly-item__label">Contact Phone</span>
								<span class="readonly-item__value">{{ customer.profile.phone || "—" }}</span>
							</div>
							<div class="readonly-item">
								<span class="readonly-item__label">Tax ID (TIN)</span>
								<span class="readonly-item__value u-font-mono">{{ customer.profile.tin || "—" }}</span>
							</div>
							<div class="readonly-item">
								<span class="readonly-item__label">Business Reg No (BRN)</span>
								<span class="readonly-item__value u-font-mono">{{ customer.profile.brn || "—" }}</span>
							</div>
							<div class="readonly-item">
							...
							<div class="readonly-item">
								...
								<span class="readonly-item__value u-font-mono">{{ customer.profile.identityNo || "—" }}</span>
							</div>
						</div>
					</div>
					...
					<div class="panel-card panel-card--readonly mt-lg">
						<div class="panel-card__header">
							<h2>
								<i class="mdi mdi-map-marker-outline u-text-primary"></i> Primary Registered Address (Address Table Linked)
							</h2>
						...
						<div class="readonly-grid" v-if="customer.address || customer.addressCode">
							<div class="readonly-item readonly-item--full" v-if="customer.address?.address1">
							...
							<div class="readonly-item" v-if="customer.address?.city">
								...
								<span class="readonly-item__value">{{ customer.address.city }}</span>
							</div>
							<div class="readonly-item" v-if="customer.address?.postcode">
								<span class="readonly-item__label">Postcode</span>
								<span class="readonly-item__value u-font-mono">{{ customer.address.postcode }}</span>
							</div>
							<div class="readonly-item" v-if="customer.address?.state">
								<span class="readonly-item__label">State</span>
								<span class="readonly-item__value">{{ customer.address.state }}</span>
							</div>
							<div class="readonly-item" v-if="customer.address?.country">
								<span class="readonly-item__label">Country</span>
								<span class="readonly-item__value">{{ customer.address.country }}</span>
							</div>
							<div class="readonly-item">
								<span class="readonly-item__label">Linked Address Code (addressCode)</span>
					...
					<div class="panel-card panel-card--readonly mt-lg">
						<div class="panel-card__header">
							<h2>
								<i class="mdi mdi-database-edit-outline u-text-primary"></i> Credit & Tax Metadata
							</h2>
						</div>
						<div class="readonly-grid">
							...
							<div class="readonly-item">
								...
								<span class="readonly-item__value u-text-primary u-font-weight-bold">
									{{ customer.metadata?.creditLimit ? `$${customer.metadata.creditLimit}` : "No Limit" }}
								</span>
							</div>
							<div class="readonly-item">
								<span class="readonly-item__label">Overdue Limit</span>
								<span class="readonly-item__value u-text-danger">
									{{ customer.metadata?.overdueLimit ? `$${customer.metadata.overdueLimit}` : "No Limit" }}
								</span>
							</div>
							<div class="readonly-item">
