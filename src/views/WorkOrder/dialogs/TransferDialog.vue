...
function open() {
	form.value = {
		title: props.workOrder?.title ? `${props.workOrder.title} (Transfer)` : "",
...
async function submit() {
	isSaving.value = true;
	try {
		const { data, error } = await workOrderApi.transfer(props.woNumber, form.value);
		if (error) {
...
<template>
	<Dialog v-model="isOpen" title="Transfer Work Order" maxWidth="500px">
		<p style="margin: 0 0 16px 0; font-size: 14px; color: var(--colors-text-secondary)">
			This will close the current work order and create a new draft work order with the same details.
		</p>
		<template #footer>
			<Button variant="secondary" @click="isOpen = false">Cancel</Button>
			<Button variant="primary" @click="submit" :disabled="isSaving">Confirm Transfer</Button>
		</template>
	</Dialog>
</template>
