...
function viewFullDetails() {
    if (props.workOrder) {
        isOpen.value = false;
        router.push({ name: 'Work Order Detail', params: { id: props.workOrder.woNumber } });
    }
}
...
function toggleAssistantEngineer(code: string) {
    if (!editData.value.assistantEngineers) editData.value.assistantEngineers = [];
    const index = editData.value.assistantEngineers.indexOf(code);
    if (index > -1) {
        editData.value.assistantEngineers.splice(index, 1);
    } else {
        editData.value.assistantEngineers.push(code);
    }
}
</script>
...
<template>
    <Dialog v-model="isOpen" title="Work Order Details" maxWidth="600px">
        <div v-if="workOrder" class="details-view">
            <div class="details-section">
                ...
                <div class="details-grid">
                    <div class="detail-item">
                        ...
                        <span class="detail-value u-font-mono u-font-weight-medium">{{ workOrder.woNumber }}</span>
                    </div>
                    <div class="detail-item">
                    ...
                    <div class="detail-item">
                        ...
                        <span class="detail-value">{{ new Date(workOrder.createdAt).toLocaleString() }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="details-section" v-if="workOrder.description || workOrder.estimatedEndDate || isEditing">
                <h4>Description & Schedule</h4>
                <div class="details-grid" v-if="!isEditing">
                    <div class="detail-item" style="grid-column: 1 / -1;" v-if="workOrder.description">
                        <span class="detail-label">Work Description</span>
                        <span class="detail-value" style="white-space: pre-wrap; line-height: 1.5;">{{ workOrder.description }}</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Lead Engineer</span>
                        <span class="detail-value">{{ users.find(u => u.code === workOrder?.leadEngineer)?.name || workOrder.leadEngineer || 'Unassigned' }}</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Assistant Engineers</span>
                        <span class="detail-value">
                            {{ workOrder.assistantEngineers?.length ? workOrder.assistantEngineers.map(code => users.find(u => u.code === code)?.name || code).join(', ') : 'Unassigned' }}
                        </span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Est. Completion</span>
                        <span class="detail-value">{{ workOrder.estimatedEndDate ? new Date(workOrder.estimatedEndDate).toLocaleString() : 'Not Set' }}</span>
                    </div>
                </div>
            ...
            <div class="details-section" v-if="workOrder.location || isEditing">
                ...
                <div class="details-grid" v-if="!isEditing">
                    <div class="detail-item" style="grid-column: 1 / -1;">
                        ...
                        <span class="detail-value">{{ workOrder.location || 'Not specified' }}</span>
                    </div>
                </div>
                <div class="form-grid-edit" v-else>
                    <Textbox v-model="editData.location" label="Location" placeholder="Enter Location">
                        <template #suffix>
            ...
            <div class="details-section" v-if="workOrder.rejectedReason">
                ...
                <div class="details-grid">
                    <div class="detail-item" style="grid-column: 1 / -1;">
                        ...
                        <span class="detail-value u-text-error">{{ workOrder.rejectedReason }}</span>
                    </div>
                </div>
            </div>
        </div>
        <template #footer>
            <template v-if="!isEditing">
                ...
                <Button 
                    v-if="workOrder && ['New', 'PendingApproval', 'InProgress', 'Completed', 'Claimed'].includes(workOrder.status)" 
                    variant="primary" 
