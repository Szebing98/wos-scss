        ...
        <div class="maintenance-view__header">
            <div class="maintenance-view__title-area">
                <div class="title-with-action">
                    ...
                    <button class="icon-action-btn icon-action-btn--primary" @click="prepareNewFormat" title="Add New Format">
                        <i class="mdi mdi-plus"></i>
                    </button>
                </div>
                <p class="maintenance-view__subtitle">Configure numbering rules and generation patterns for system modules</p>
            </div>
        </div>
            ...
            <Card class="maintenance-grid__left-panel">
                <div class="list-controls">
                    <div class="search-box">
                        ...
                        <input v-model="searchString" type="text" placeholder="Search module or prefix..." class="search-box__input" />
                    </div>
                ...
                <div class="type-list">
                    ...
                    >
                        <div class="type-card__content">
                            ...
                            <span class="type-card__code">Preview: {{ generatePreview(item) }}</span>
                        </div>
                ...
                <div v-if="selectedFormat" class="detail-container">
                    ...
                    <div class="live-preview-bar">
                        <div class="live-preview-bar__left">
                            ...
                            <span class="live-preview-bar__value u-font-mono">{{ generatePreview(selectedFormat) }}</span>
                        </div>
                    ...
                    <Card class="panel-card--readonly mt-md">
                        <div class="panel-card__header">
                            ...
                            <button class="icon-action-btn icon-action-btn--edit" @click="openEditModal" title="Edit Configuration">
                        ...
                        <div class="readonly-grid">
                            <div class="readonly-item">
                                ...
                                <span class="readonly-item__value u-font-weight-bold">{{ selectedFormat.module }}</span>
                            </div>
                            <div class="readonly-item">
                                <span class="readonly-item__label">Prefix Text</span>
                                <span class="readonly-item__value u-font-mono">{{ selectedFormat.prefix }}</span>
                            </div>
                            <div class="readonly-item">
                                <span class="readonly-item__label">Timestamp Format</span>
                                <span class="readonly-item__value">{{ selectedFormat.dateFormat || 'None (Disable Timestamp)' }}</span>
                            </div>
                            <div class="readonly-item">
                                <span class="readonly-item__label">Delimiter Splitting</span>
                                <span class="readonly-item__value u-font-mono">{{ selectedFormat.delimiter || 'None (Seamless String)' }}</span>
                            </div>
                            <div class="readonly-item">
                                <span class="readonly-item__label">Running No. Digits</span>
                                <span class="readonly-item__value">{{ selectedFormat.padding }} Digits (e.g. {{ '0'.repeat(selectedFormat.padding - 1) }}1)</span>
                            </div>
                            <div class="readonly-item">
                                <span class="readonly-item__label">Next Serial Counter</span>
                                <span class="readonly-item__value u-text-primary u-font-weight-bold">{{ selectedFormat.nextNumber }}</span>
                            </div>
                        </div>
                        
                        <div class="panel-card__footer-actions" style="margin-top: 24px; padding-top: 16px; border-top: 1px dashed var(--colors-surface-border);">
                            <button class="action-btn action-btn--danger-text" @click="deleteFormat(selectedFormat)">
        ...
        <Dialog v-model="isDialogOpen">
            ...
            <div class="form-grid">
                <div class="form-group">
                    <label class="form-group__label">Module Code <span class="u-required">*</span></label>
                    <input v-model="formData.module" type="text" class="form-group__input" :disabled="!isNewRecord" placeholder="e.g. INV, PO, WO" />
                </div>
                <div class="form-group">
                    <label class="form-group__label">Prefix <span class="u-required">*</span></label>
                    <input v-model="formData.prefix" type="text" class="form-group__input" placeholder="e.g. TECH, MYS" />
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input v-model="formData.delimiter" type="text" maxlength="5" class="form-group__input u-font-mono" placeholder="e.g. -, /, _" />
                </div>
                <div class="form-group">
                    <label class="form-group__label">Serial Code Width</label>
                    <input v-model.number="formData.padding" type="number" min="1" max="10" class="form-group__input" />
                </div>
                <div class="form-group">
                    <label class="form-group__label">Next Serial Counter</label>
                    <input v-model.number="formData.nextNumber" type="number" min="1" class="form-group__input u-text-primary u-font-weight-bold" />
                </div>
            ...
            <template #footer>
                <button class="action-btn action-btn--text" @click="isDialogOpen = false">Cancel</button>
                <button class="action-btn action-btn--primary" @click="saveFormat">Save Configuration</button>
            </template>
        </Dialog>
    </div>
</template>
...
const formData = ref<DocNoFormatModel>({
    id: 0, module: "", prefix: "", dateFormat: "YYYYMM", delimiter: "-", padding: 4, nextNumber : 1, isActive: true
});

// Mock 规则仓库
const formats = ref<DocNoFormatModel[]>([
    { id: 1, module: "INV", prefix: "INV", dateFormat: "YYYYMM", delimiter: "-", padding: 4, nextNumber: 1, isActive: true },
    { id: 2, module: "PO", prefix: "PO", dateFormat: "YYYYMMDD", delimiter: "/", padding: 5, nextNumber: 25, isActive: true },
    { id: 3, module: "DO", prefix: "DO", dateFormat: "YYMM", delimiter: "", padding: 4, nextNumber: 120, isActive: true }
]);
...
const filteredFormats = computed(() => {
    ...
    return formats.value.filter(x => {
        ...
        const matchesStatus = filterActive.value === "all" ||
...
function prepareNewFormat() {
    ...
    formData.value = { id: 0, module: "", prefix: "", dateFormat: "YYYYMM", delimiter: "-", padding: 4, nextNumber: 1, isActive: true };
...
function saveFormat() {
    ...
    if (isNewRecord.value) {
        // 重复校验
        const duplicate = formats.value.some(x => x.module.toLowerCase() === formData.value.module.toLowerCase());
        if (duplicate) {
...
.title-with-action {
    ...
    .icon-action-btn--primary {
        background-color: rgba(80, 88, 242, 0.08); color: var(--colors-primary-deepblue);
        border-radius: 50%; width: 32px; height: 32px;
        &:hover { background-color: var(--colors-primary-deepblue); color: white; }
    }
}

// 核心 Master-Detail 左右栅格比例
.maintenance-grid {
    display: grid; grid-template-columns: 4.2fr 7.8fr; gap: var(--spacing-lg); align-items: start;
    @media (max-width: 960px) { grid-template-columns: 1fr; }

    &__left-panel {
        padding: var(--spacing-md); height: 560px; display: flex; flex-direction: column; gap: 12px;
...
// 🌟 实时动态预览条 (Live Preview Ribbon)
.live-preview-bar {
    background-color: var(--colors-surface-card); border-left: 5px solid var(--colors-brand-primary);
    border-radius: 8px; padding: 14px 20px; display: flex; justify-content: space-between; align-items: center;
    box-shadow: inset 0 1px 3px rgba(0, 0, 0, 0.02), 0 2px 8px rgba(0, 0, 0, 0.01);
    transition: all 0.2s ease;

    &__left { display: flex; flex-direction: column; gap: 4px; }
    &__label { font-size: 10px; font-weight: 700; color: var(--colors-text-muted); letter-spacing: 0.08em; }
    &__value { font-size: 22px; font-weight: 800; color: var(--colors-brand-primary); letter-spacing: -0.02em; }
}

// 弹窗特有迷你动态流预览块
.modal-box__mini-preview {
    background-color: var(--colors-surface-background); color: var(--colors-brand-primary); font-size: 16px; font-weight: 700;
    padding: 12px 20px; text-align: center; border-left: 4px solid var(--colors-brand-primary);
    margin-bottom: var(--spacing-lg); border-radius: 6px; letter-spacing: 0.02em;
}
...
.panel-card--readonly {
    ...
    .icon-action-btn--edit {
        font-size: 13px; font-weight: 600; color: var(--colors-brand-primary);
        background: var(--colors-surface-hover); padding: 4px 12px; border-radius: 6px; gap: 4px; display: inline-flex; align-items: center; border: none; cursor: pointer;
        &:hover { background: var(--colors-brand-primary); color: white; }
    }
}

.readonly-grid {
    display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px var(--spacing-md); padding: 8px 0;
    @media (max-width: 640px) { grid-template-columns: 1fr; }
}

.readonly-item {
    display: flex; flex-direction: column; gap: 4px;
    &__label { font-size: 12px; font-weight: 600; color: var(--colors-text-muted); text-transform: uppercase; letter-spacing: 0.02em; }
    &__value { font-size: 14px; font-weight: 600; color: var(--colors-text-primary); }
}
...
.type-card {
    ...
    &__code { font-size: 11px; color: var(--colors-text-muted); font-family: monospace; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
// 基础原子小表单与公用组件
.list-controls { display: flex; align-items: center; gap: var(--spacing-sm); margin-bottom: var(--spacing-sm); }
...
.form-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--spacing-md); @media (max-width: 540px) { grid-template-columns: 1fr; } }
.form-group {
...
.action-btn {
    border: none; border-radius: 6px; font-size: 13px; font-weight: 600; padding: 8px 16px; cursor: pointer; display: inline-flex; align-items: center; gap: 6px;
    &--primary { background-color: var(--colors-brand-primary); color: white; &:hover { opacity: 0.9; } }
    &--text { background: transparent; color: var(--colors-text-muted); &:hover { background: var(--colors-surface-hover); } }
    &--danger-text { background: transparent; color: #ef4444; padding: 4px 8px; font-size: 12px; &:hover { background: rgba(239, 68, 68, 0.1); } }
}
...
.empty-state { height: 100%; min-height: 400px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: var(--spacing-md); background: var(--colors-surface-card); border: 1px solid var(--colors-surface-border); border-radius: 12px; color: var(--colors-text-muted); &__icon { font-size: 4rem; opacity: 0.15; } p { font-size: 14px; font-weight: 500; } }

.mt-md { margin-top: var(--spacing-md); }
.u-font-mono { font-family: monospace; }
.u-font-weight-bold { font-weight: 700; }
.u-text-primary { color: var(--colors-brand-primary); }
