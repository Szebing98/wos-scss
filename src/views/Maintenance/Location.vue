...
function saveChanges() {
	...
	const idx = countries.value.findIndex(
		(c) => c.alpha3Code === selectedCountry.value?.alpha3Code,
	);
	if (idx !== -1) {
	...
	<div class="location-view">
		<div class="location-view__header">
			<div class="location-view__title-area">
				...
				<p class="location-view__subtitle">
					Official LHDN MyInvois Country & State Standards
				</p>
			</div>
		...
		<Dialog v-model="isDrawerOpen">
			...
			<div v-if="selectedCountry?.alpha3Code === 'MYS'" class="alert alert--warning">
				...
				<div class="alert__content">
					<strong>Malaysia (MYS) Detected:</strong> These 16 state codes are strictly
					mandatory for MyInvois validation. Delete is forbidden.
				</div>
			</div>

			<div
				class="drawer-box__sub-title-row"
				style="display: flex; justify-content: space-between; align-items: center"
			>
				<h3 style="font-size: 15px; font-weight: 600; margin: 0">
					States / Regions ({{ selectedCountry?.subNodes.length }})
				</h3>
				<button
